...
class CDNSLookup
{
	...
	bool DNSLookup(unsigned long ulDNSServerIP, char *szDomainName, std::vector<unsigned long> *pveculIPList = NULL, std::vector<std::string> *pvecstrCNameList = NULL,uint16_t type = MX, unsigned long ulTimeout = 1000, unsigned long *pulTimeSpent = NULL);
	bool DNSLookup(unsigned long ulDNSServerIP, char *szDomainName, std::vector<std::string> *pvecstrIPList = NULL, std::vector<std::string> *pvecstrCNameList = NULL,uint16_t type =MX,unsigned long ulTimeout = 1000, unsigned long *pulTimeSpent = NULL);

    private:
	bool Init();
	bool UnInit();
	bool DNSLookupCore(unsigned long ulDNSServerIP, char *szDomainName, std::vector<unsigned long> *pveculIPList, std::vector<std::string> *pvecstrCNameList, unsigned long ulTimeout, unsigned long *pulTimeSpent,uint16_t type);
	bool SendDNSRequest(sockaddr_in sockAddrDNSServer, char *szDomainName,uint16_t type);
